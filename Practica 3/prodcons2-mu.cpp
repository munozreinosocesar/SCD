...
{
  static int contador = (orden *num_items) - 1;
  contador++;
  // sleep_for( milliseconds( aleatorio<10,100>()) );
   cout << "Productor " << orden /num_items << " ha producido valor " << contador << endl << flush;
   return contador ;
}
// ---------------------------------------------------------------------
...
{
   for ( unsigned int i= orden * num_items ; i < num_items/num_prod ; i++ ){

      // producir valor
      int valor_prod = producir(orden);
      // enviar valor
      cout << "Productor " << orden << " va a enviar valor " << valor_prod << endl << flush;
      MPI_Send( &valor_prod, 1, MPI_INT, id_buffer, etiq_prod, MPI_COMM_WORLD);
   }
}
// ---------------------------------------------------------------------
...
void funcion_buffer()
{
   int        buffer[tam_vector],      // buffer con celdas ocupadas y vacías
              ...
              id_emisor_aceptable ,
              tag,
              destino;    // identificador de emisor aceptable
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      MPI_Recv( &valor, 1, MPI_INT, id_emisor_aceptable, tag, MPI_COMM_WORLD, &estado );
      ...
      switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
      {
         ...
         case etiq_cons: // si ha sido el consumidor: extraer y enviarle
            ...
            destino = aleatorio<num_prod+1,num_prod + num_cons>();
            MPI_Ssend( &valor, 1, MPI_INT, destino, etiq_cons, MPI_COMM_WORLD);
            break;
      }
   }
}
...
int main( int argc, char *argv[] )
{
   ...
   if ( num_procesos_esperado == num_procesos_actual )
   {
       if ( id_propio < num_prod){
         cout << "PRODUCTOR " << id_propio << " ORDEN "<< id_propio << endl;
         ...
         cout << "CONSUMIDOR " << id_propio << " ORDEN "<< id_propio << endl;
